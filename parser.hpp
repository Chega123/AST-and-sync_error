...
static int line_global = 1;
static int col_global = 0;
...
struct token {
    ...
    token(string N_) : token_name(N_), text("tesfsdft"), line(line_global), col(col_global) {}
    token(string N_, string T_) : token_name(N_), text(T_), line(line_global), col(col_global) {}
...
struct ASTNode {
  ...
  ASTNode(std::string token, std::string w) : token_name(std::move(token)), word(std::move(w)) {}

  void addChild(ASTNode child) {
    children.push_back(std::move(child));
  }
  // Método para agregar un hijo con un nombre específico
  void addChild(ASTNode child, const std::string& name) {
    child.token_name = name; // Cambiar el nombre antes de agregarlo
    children.push_back(std::move(child));
}
